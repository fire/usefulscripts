...
yum install glibc-devel libgcc libX11-devel freetype-devel gnutls-devel libxml2-devel libjpeg-turbo-devel libpng-devel libXrender-devel -y 2>&1 >> $log
...
cd /usr/src 2>&1 >> $log
curl http://dl.winehq.org/wine/source/2.0/wine-${ver}.tar.bz2 -o wine-${ver}.tar.bz2 2>&1 >> $log
